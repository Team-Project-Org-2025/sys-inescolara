-- ============================================================================
-- SISTEMA DE GESTIÓN VIVERO INESColara
-- Base de Datos Definitiva v2.0
-- Fecha: 2026-06-10
-- ============================================================================
-- Este archivo contiene el esquema COMPㄱLETO y DEFINITIVO de ambas bases de
-- datos. Reemplaza todos los scripts individuales de la carpeta `scripts/`.
--
-- Cambios respecto al esquema anterior:
--  1. Tablas ELIMINADAS (redundantes/sin uso):
--     - `asistencia`              ─ reemplazada por lógica en tareas
--     - `movimiento_insumo`       ─ vacía, solapada con compra + ajuste_inventario
--     - `movimiento_insumo_detalle` ─ vacía, solapada con compra_detalle
--     - `planta_precio_vigente`   ─ redundante con `calculo_precio`
--     - `sesiones_activas`        ─ vacía, sin uso
--  2. `compra_detalle`: FK polimórfico (tipo_item + id_item) reemplazado por
--     columnas FK reales: id_insumo, id_herramienta, id_planta
--  3. `compra`: nueva columna `fecha_recepcion`
--  4. `recoleccion_semillas`: nueva columna `activo`
--  5. `recoleccion_semillas_detalle`: nueva columna `id_insumo` + FK a `insumo`
-- ============================================================================
...
-- --------------------------------------------------------------------------
-- TABLAS ELIMINADAS (documentación)
-- Ya no forman parte del esquema. Se listan aquí como referencia.
-- --------------------------------------------------------------------------
-- asistencia              → reemplazada por control en asignar_tarea
-- movimiento_insumo       → vacía, usar compra + ajuste_inventario
-- movimiento_insumo_detalle → vacía, usar compra_detalle
...
CREATE TABLE IF NOT EXISTS `calculo_precio` (
  ...
  KEY `id_lote` (`id_lote`),
  CONSTRAINT `fk_calculo_lote` FOREIGN KEY (`id_lote`) REFERENCES `lote` (`id_lote`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  COMMENT='Cálculo de precio por lote.';
...
CREATE TABLE IF NOT EXISTS `trabajadores` (
  ...
  UNIQUE KEY `uq_trabajador_cedula` (`cedula_trabajador`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
...
CREATE TABLE IF NOT EXISTS `compra_detalle` (
  ...
  `id_insumo`        INT(11)       DEFAULT NULL COMMENT 'FK real a insumo (antes tipo_item+id_item)',
...
CREATE TABLE IF NOT EXISTS `movimiento_planta` (
  ...
  CONSTRAINT `fk_movplanta_trabajador` FOREIGN KEY (`id_trabajador_gestor`) REFERENCES `trabajadores` (`id_trabajador`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
...
CREATE TABLE IF NOT EXISTS `ajuste_inventario` (
  ...
  CONSTRAINT `fk_ajuste_trabajador` FOREIGN KEY (`id_trabajador`) REFERENCES `trabajadores` (`id_trabajador`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
...
-- --------------------------------------------------------------------------
-- sesiones_activas ELIMINADA (vacía, sin uso)
-- --------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS `usuarios` (
  ...
  UNIQUE KEY `uq_usuario_trabajador` (`id_trabajador_ref`),
  KEY `id_rol` (`id_rol`),
