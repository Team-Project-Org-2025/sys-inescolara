const SkeletonHelper = {
  createTableSkeleton(rows = 5, cols = 6) {
    ...
    for (let i = 0; i < rows; i++) {
      ...
      for (let j = 0; j < cols; j++) {
        ...
        const width = j === 0 ? '60px' : j === cols - 1 ? '80px' : '100%';
  ...
  hideModalSkeleton(contentId, newContent) {
    ...
    $content.fadeOut(150, function () {
      $(this).html(newContent).addClass('content-loaded').fadeIn(150);
    });
  },
  ...
  showSearchSkeleton(resultContainerId, items = 3) {
    $(`#${resultContainerId}`).html(this.createListSkeleton(items)).show();
  },
  ...
  hideAll() {
    $('.skeleton-table-row').fadeOut(200, function () {
      $(this).remove();
    });
    $('.skeleton-modal-content').fadeOut(200, function () {
      $(this).remove();
    });
    $('.skeleton-form-content').fadeOut(200, function () {
      $(this).remove();
    });
  },
};
