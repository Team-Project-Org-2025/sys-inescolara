...
class Batch extends Database implements ReadableInterface, DeletableInterface
{
    ...
    {
        $stmt = $this->db->prepare("SELECT COUNT(*) FROM lotes WHERE id_lote = :id");
        $stmt->execute([':id' => $id]);
    ...
    {
        $stmt = $this->db->prepare("DELETE FROM lotes WHERE id_lote = :id");
    ...
    {
        $stmt = $this->db->prepare("INSERT INTO lotes (id_planta, fecha_siembra, cantidad_inicial, cantidad_actual, estado, ubicacion, imagen) VALUES (:id_planta, :fecha_siembra, :cantidad_inicial, :cantidad_actual, :estado, :ubicacion, :imagen)");
        return $stmt->execute([
            ':id_planta' => $id_planta,
    ...
    public function update($id, $id_planta, $fecha_siembra, $cantidad_inicial, $cantidad_actual, $estado, $ubicacion, $imagen = null)
    {
        ...
        $stmt = $this->db->prepare("UPDATE lotes SET id_planta = :id_planta, fecha_siembra = :fecha_siembra, cantidad_inicial = :cantidad_inicial, cantidad_actual = :cantidad_actual, estado = :estado, ubicacion = :ubicacion, imagen = :imagen WHERE id_lote = :id");
        return $stmt->execute([
            ':id' => $id,
            ':id_planta' => $id_planta,
            ':fecha_siembra' => $fecha_siembra,
